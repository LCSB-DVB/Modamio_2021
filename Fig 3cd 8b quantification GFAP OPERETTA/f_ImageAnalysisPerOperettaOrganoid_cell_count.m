function  [ObjectsThisOrganoid] = f_ImageAnalysisPerOperettaOrganoid_cell_count(Label, ch1, ch2, ch3, ch4, ChannelNames, PreviewPath);
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here

    % vol(ch1, 0, 500) % Alexa 488 >>> TUJ1 
    % vol(ch2, 0, 4000) % Alexa 647 >>> GFAP
    % vol(ch3, 0, 5000) % HOECHST 33342 >>> Hoechst imtool(max(ch2, [], 3))
    % vol(ch4, 0, 5000) % TRITC >>> S100b
   ...
   TUJ1Mask = [];
    ...
    NucleiMask = ch3DoG > 50; %vol(NucleiMask)
    ...
    GFAPMask = ch2MedFilt > 1000; %previously 2000
    GFAPMask = bwareaopen(GFAPMask, 200);
...
%                         %vol(SurfaceGFAP)
   ...
   TUJ1Mask = ch1MedFilt > 1600; 
   TUJ1Mask = bwareaopen(TUJ1Mask, 200);
    %vol(TUJ1Mask)
...
%     DoubleMasksPercent = (sum(PeriNucObjectsCompactDoubleMasks.DoubleMaskspos)/height(PeriNucObjectsCompactDoubleMasks))*100;
    ...
    PreviewS100b = imoverlay2(imadjust(max(ch4, [], 3), [0 0.02]), bwperim(max(S100bMask,[],3)), [0 0 1]);
    PreviewS100b = imoverlay2(PreviewS100b, BarMask, [1 1 1]);
    %imtool(PreviewS100b)
    
    PreviewTuj1 = imoverlay2(imadjust(max(ch1, [], 3), [0 0.07]), bwperim(max(TUJ1Mask,[],3)), [0 0 1]);
    PreviewTuj1 = imoverlay2(PreviewTuj1, BarMask, [1 1 1]);
    %imtool(PreviewTuj1)
    
    PreviewDoubleMask = imoverlay2(imadjust(max(ch2, [], 3), [0 0.07]), bwperim(max(DoubleMasks,[],3)), [0 0 1]);
    ...
    ObjectsThisOrganoid.DoubleMasks = sum(DoubleMasks(:));
    ...
    ObjectsThisOrganoid.DoubleMaskByNuc = sum(DoubleMasks(:)) / sum(NucleiMask(:));
   
    ObjectsThisOrganoid.Tuj1ByNucAlive = sum(TUJ1Mask(:)) / sum(NucMaskAlive(:));
    ObjectsThisOrganoid.S100bMaskNucAlive = sum(S100bMask(:)) / sum(NucMaskAlive(:));
    ObjectsThisOrganoid.GFAPMaskByNucAlive = sum(GFAPMask(:)) / sum(NucMaskAlive(:));
    ObjectsThisOrganoid.DoubleMaskByNucAlive = sum(DoubleMasks(:)) / sum(NucMaskAlive(:));
